...
def _convert_to_slack_message(summary: dict, account_info: tuple) -> dict:
    ...
    result = {"blocks": [{"type": "section", "text": {"type": "mrkdwn", "text": ""}}]}
    ...
    if summary["event_name"] == "ConsoleLogin":
        message = (
            f":warning: *{summary['identity']}* logged in *{account_str}* \n"
        ...
        message = (
            f":warning: *{summary['identity']}* created new resources on "
...
def process_event_by_service(record: dict, set_tags: bool = False) -> dict:
    ...
    try:
        ...
        return {
            "error": f"Not supported service: {service_name}, Event ID: {record['eventID']}"
        }
...
def notify_slack(summary: dict, account_info: tuple):
    ...
    if (
        "SLACK_WEBHOOK_URL" in os.environ.keys()
        and os.environ["SLACK_WEBHOOK_URL"] != "DISABLED"
    ):
        req = request.Request(
...
def build_result(record: dict, set_tags: bool = False) -> dict:
    ...
    if record["eventName"] == "ConsoleLogin":
        result = {"resource_id": [record["responseElements"]["ConsoleLogin"]]}
    else:
