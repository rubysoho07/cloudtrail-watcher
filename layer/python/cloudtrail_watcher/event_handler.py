...
from cloudtrail_watcher.utils import (
    notify_slack,
    notify_sns,
    get_account_alias,
    build_result,
...
def handler(event, context):
    ...
    for record in data["Records"]:
        try:
            ...
            if disable_autoscaling_alarm and result["identity"].startswith(
                "assumed-role/AWSServiceRoleForAutoScaling"
            ):
                continue
            ...
            print(
                f"(ClientError) event ID: {record['eventID']}, event name: {record['eventName']}, "
            ...
            if record is not None:
                print(
                    f"(Cannot process event record) event ID: {record['eventID']}, eventName: {record['eventName']}, "
                    f"Error: {e}"
                )
