...
from .common import get_user_identity
...
def _process_run_instances(event: dict, set_tags: bool = False) -> list:
    ...
    if set_tags:
        ...
        if "tagSpecificationSet" in event["requestParameters"].keys():
            _check_instance_tag_requests(
                event["requestParameters"]["tagSpecificationSet"], has_user_tag
            )
        ...
        for resource_id in resource_ids:
            ...
            if (
                has_user_tag["network-interface"] is False
                and instance.network_interfaces is not None
            ):
...
def _process_authorize_security_group_egress(
    event: dict, set_tag: bool = False
) -> list:
    resource_ids = [event["requestParameters"]["groupId"]]
    return resource_ids


def _process_authorize_security_group_ingress(
    event: dict, set_tag: bool = False
) -> list:
    resource_ids = [event["requestParameters"]["groupId"]]
    return resource_ids
...
def _process_modify_security_group_rules(event: dict, set_tag: bool = False) -> list:
    resource_ids = [
        event["requestParameters"]["ModifySecurityGroupRulesRequest"]["GroupId"]
    ]
    return resource_ids
...
def process_event(event: dict, set_tag: bool = False) -> dict:
    ...
    if event["responseElements"] is None:
        result["error"] = (
            f"response is null: eventName - {event['eventName']}, eventID: {event['eventID']},"
        )
        return result
    ...
    if event["eventName"] == "RunInstances":
        ...
        result["resource_id"] = _process_authorize_security_group_ingress(
            event, set_tag
        )
