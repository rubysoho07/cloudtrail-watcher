...
def _process_create_cluster(event: dict, set_tag: bool = False) -> list:
    ...
    if set_tag:
        if (
            "tags" not in event["responseElements"].keys()
            or check_contain_mandatory_tag_list(event["responseElements"]["tags"])
            is False
        ):
...
def process_event(event: dict, set_tag: bool = False) -> dict:
    ...
    if event["responseElements"] is None:
        result["error"] = (
            f"response is None: check CloudTrail event - {event['eventID']}"
        )
        return result
