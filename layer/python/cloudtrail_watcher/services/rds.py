...
from .common import (
    get_user_identity,
    check_contain_mandatory_tag_list,
    get_service_name,
)
...
def process_event(event: dict, set_tag: bool = False) -> dict:
    ...
    if (
        "engine" in event["requestParameters"].keys()
        and event["requestParameters"]["engine"] == "docdb"
    ):
    ...
    if "errorCode" in event.keys():
        result["error"] = (
            f"{event['errorCode']}: check CloudTrail event - {event['eventID']}"
        )
        return result
